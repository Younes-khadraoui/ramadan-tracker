...
import { loadData, saveData, generateId, cycleCell, RAMADAN_DAYS } from '@/lib/storage';
...
export default function RamadanTracker() {
  ...
  const getChildren = (parentId: string) =>
    data.tasks.filter(t => t.parentId === parentId).sort((a, b) => a.order - b.order);

  const hasChildren = (id: string) => data.tasks.some(t => t.parentId === id);
      ...
      <div key={task.id}>
        ...
        >
          ...
          >
            ...
            {editingId === task.id ? (
              ...
              <span
                ...
                dir="auto"
              >
                {task.name || <span className="text-muted-foreground italic">اضغط للتسمية</span>}
              </span>
            )}
          ...
          {days.map(day => {
            ...
            const burstType = burst ? (animatingBurst?.endsWith('done') ? 'done' : 'missed') : null;
        ...
        {/* Children */}
        {isParent && !isCollapsed && children.map(child => renderTaskRow(child, depth + 1))}
      </div>
    );
  };
